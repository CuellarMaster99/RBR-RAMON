...
Pasos:
  1. Sube uno_pid_tb6612.ino al Arduino Uno
  2. Conecta el mando Xbox 360 (receptor USB o cable)
  3. Cambia SERIAL_PORT con el COM que usa tu Arduino
...
MAX_RPM   = 150.0
DPAD_RPM  = 100.0          # RPM fija para D-Pad
DEAD_JOY  = 0.08          # zona muerta joystick
DEAD_TRIG = 0.03          # zona muerta gatillos

# Índices de ejes — Xbox 360 estándar en Windows/Linux
AX_JOY_IZQ_X = 0          # Joystick izquierdo horizontal
AX_LT        = 4          # Gatillo izquierdo (LT)
AX_RT        = 5          # Gatillo derecho  (RT)

# Cada cuántos ciclos (50 Hz) se reenvía el setpoint al Arduino
# Necesario para el watchdog: si no llegan comandos en 2s → STOP
RESEND_EVERY = 10          # cada 200 ms
...
def connect_serial(port, baud, retries=5):
    ...
    for i in range(retries):
        try:
            ...
            time.sleep(2)          # esperar al Arduino (reset por DTR)
            _serial_ok = True
            ...
            if "RPM1:" in raw and "Target1:" in raw:
                ...
                for tok in raw.split("\t"):
                    if ":" in tok:
                        k, v = tok.split(":", 1)
                        p[k.strip()] = v.strip()
                rpm1_live = float(p.get("RPM1",    0))
                rpm2_live = float(p.get("RPM2",    0))
                tgt1_live = float(p.get("Target1", 0))
                tgt2_live = float(p.get("Target2", 0))
                err1_live = float(p.get("Err1",    0))
                err2_live = float(p.get("Err2",    0))
                out1_live = float(p.get("Out1",    0))
                out2_live = float(p.get("Out2",    0))
                d_rpm1.append(rpm1_live); d_rpm2.append(rpm2_live)
                d_tgt1.append(tgt1_live); d_tgt2.append(tgt2_live)
                d_err1.append(err1_live); d_err2.append(err2_live)
            elif raw and ">>" not in raw and "+" not in raw[:2]:
                pass   # ignorar eco de comandos
        except Exception:
            pass
...
def apply_steering(base_rpm: float, joy_x: float):
    """Mezcla velocidad base + dirección con joystick izquierdo."""
...
class Dashboard(QWidget):
    def __init__(self, joy):
        super().__init__()
        self.joy = joy
        self.setWindowTitle(f"Arduino Uno  PID Robot  —  Xbox 360  |  {SERIAL_PORT}")
    ...
    def _ctrl_tick(self):
        ...
        if rt > 0.5 and lt > 0.5:
            ...
            self._mode_str   = f"D-PAD ←  M1={m1o:+.0f} M2={m2o:+.0f}"
    ...
    def _build_ui(self):
        ...
        # Header
        hdr = QHBoxLayout()
        title = QLabel("ARDUINO UNO  —  PID ROBOT  TB6612FNG  Xbox 360")
        title.setFont(QFont("Courier New",14,QFont.Bold))
        title.setStyleSheet("color:#58a6ff;letter-spacing:1px;")
        sub = QLabel(f"Serial: {SERIAL_PORT}  |  RT=avance  LT=retroceso  D-Pad=movimiento  Joy IZQ=dirección")
    ...
    def _xbox_panel(self):
        ...
        vl  = QVBoxLayout(box); vl.setSpacing(8)

        # Mapa de controles
        ctrl_map = [
            ("RT  (gatillo der)", "Avance proporcional  0–150 RPM", "#3fb950"),
            ("LT  (gatillo izq)", "Retroceso proporcional  0–150 RPM", "#f85149"),
            ("D-Pad  ↑ ↓",        f"Avance/retroceso fijo  ±{DPAD_RPM:.0f} RPM", "#58a6ff"),
            ("D-Pad  ← →",        "Giro en sitio", "#d29922"),
            ("Joy IZQ  X",        "Dirección (mezcla sobre RT/LT)", "#c9d1d9"),
        ]
        for ctrl, desc, color in ctrl_map:
            row = QHBoxLayout()
            cl = QLabel(ctrl); cl.setFont(QFont("Courier New",9,QFont.Bold))
            cl.setStyleSheet(f"color:{color};"); cl.setFixedWidth(130)
        ...
        self.rpm_inp.setFont(QFont("Courier New",11)); self.rpm_inp.setFixedWidth(70)
        ...
        sb.setStyleSheet("QGroupBox{background:#0d1117;border:1px solid #1f6feb;"
                         "border-radius:6px;margin-top:6px;padding:8px;color:#58a6ff;"
                         "font-family:'Courier New';font-size:9pt;}"
                         "QGroupBox::title{subcontrol-origin:margin;left:10px;padding:0 4px;}")
        sl = QVBoxLayout(sb)
        kr = QHBoxLayout()
        kl = QLabel("Kp sync:"); kl.setFont(QFont("Courier New",10))
        kl.setStyleSheet("color:#8b949e;")
        self.kpsync = QLineEdit("0.5")
        self.kpsync.setFont(QFont("Courier New",10)); self.kpsync.setFixedWidth(70)
        self.kpsync.setAlignment(Qt.AlignCenter)
        self.kpsync.setStyleSheet("background:#0d1117;color:#c9d1d9;"
        ...
        stop_btn.clicked.connect(lambda: (set_motors(0,0,force=True),
                                          serial_send("stop")))
    ...
    def _upd_disp(self):
        ...
        self.disp1.setStyleSheet(f"color:{c1};background:#0d1117;"
                                 f"border:1px solid #30363d;border-radius:10px;padding:10px;")
        self.disp2.setText(f"M2 IZQUIERDO [{d2}]\n{rpm2_live:+.1f} RPM")
        self.disp2.setStyleSheet(f"color:{c2};background:#0d1117;"
                                 f"border:1px solid #30363d;border-radius:10px;padding:10px;")
        # Modo
    ...
    def _upd_plot(self):
        ...
        self.ax_rpm.plot(xs,r1,color="#58a6ff",lw=2.0,label="M1")
        self.ax_rpm.plot(xs,r2,color="#f78166",lw=2.0,label="M2")
        self.ax_rpm.set_title("RPM vs Target",color="#8b949e",fontsize=9,pad=4)
        ...
        self.ax_err.fill_between(xs,e1,alpha=0.15,color="#58a6ff")
        self.ax_err.fill_between(xs,e2,alpha=0.15,color="#f78166")
        self.ax_err.plot(xs,e1,color="#58a6ff",lw=1.8,label="Err M1")
        self.ax_err.plot(xs,e2,color="#f78166",lw=1.8,label="Err M2")
        self.ax_err.set_title("Error PID",  color="#8b949e",fontsize=9,pad=4)
        self.ax_err.set_ylabel("error",     color="#8b949e",fontsize=8)
...
def main():
    ...
    # Iniciar pygame y detectar mando
    pygame.init()
